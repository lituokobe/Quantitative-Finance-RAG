...
- A call option is a contract that gives the owner the right, but not the obligation, to buy an underlying security at a specific price within a specified time frame.
...
- Call options may be purchased for speculation, sold for income purposes, or used for tax management.
...
The contract is an option (a choice) to buy the asset at a specific price by a certain date. The date is called the expiration date (known as expiry), and the asset is called the underlying asset (also called "the underlying").
...
Even if the company doesn't report a positive earnings beat (or one that doesn't meet market expectations) and its share price declines, the maximum losses a call option's buyer will bear are limited to the premiums paid for the option.
...
What happens if ABC's share price declines below \$50 by Nov. 30? Since your options contract is a right, not an obligation, to purchase ABC shares, you can choose not to exercise it, meaning you will not buy ABC's shares. In this case, your losses will be limited to the premium you paid for the option.

$$\text{Payoff} = \text{Spot price} - \text{Strike price}$$
...
Using the formula above, your income would be \$1 if ABC's spot price is \$49 on Nov. 30.
...
Covered calls work because if the stock rises above the strike price, the buyer can use their option to purchase it from you at that lower price. This means the option writer doesn't profit from the stock's movement above the strike price. The writer's maximum profit on the option is the premium received.
...
For example, an investor may own 100 shares of XYZ stock and may be liable for a large unrealized capital gain. Not wanting to trigger a taxable event, shareholders may use options to reduce the exposure to the underlying security without actually selling it.
...
Although options profits are classified as short-term capital gains, the calculation of tax liability varies based on the option strategy and holding period.
...
Think of a call option as a reservation to buy something at a set price before a certain date. You pay a small fee to make that reservation, and if the price of that item (usually a stock) goes up, you can still buy it at the lower price you reserved. If you do, you'll make a profit.

If the price doesn't go up, you don't have to buy it, and the only thing you lose out on is the fee you paid to reserve it. People use call options to try to make money when they think a stock's price will rise. The overall goal is to benefit from price changes in a stock without having to buy it right away.
...
Buying calls is bullish because the buyer only profits if the price of the shares rises. Conversely, selling call options is bearish because the seller profits if the shares don't rise. Whereas the profits of a call buyer are theoretically unlimited, the profits of a call seller are limited to the premium they receive when they sell the calls.
