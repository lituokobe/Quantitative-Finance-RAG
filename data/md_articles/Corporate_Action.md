# Understanding Common Corporate Actions & Their Impact on Stakeholders
...
- Corporate actions are events initiated by a company that bring significant changes affecting stakeholders like shareholders and bondholders.
- Some corporate actions are mandatory, such as stock splits and company name changes, and are automatically applied, while others are voluntary, requiring shareholder input, like tender offers and rights issues.
- Mergers and acquisitions involve the combination of companies, potentially forming a new entity or one company acquiring another.
- Spin-offs create a new independent company from the parent company’s assets, often involving a rights issue for current shareholders.
- Adopting corporate actions can result in operational and structural changes that the company's board of directors and shareholders usually approve.

## The Impact of Corporate Actions on Stakeholders
...
Shareholder-approved corporate actions are usually listed in a firm's proxy statement, filed before the company's annual meeting. Corporate actions can also be revealed in 8-K filings for material events.

## Key Types of Corporate Actions Explained

Corporate actions, like stock splits, dividends, mergers, acquisitions, rights issues, and spin-offs, require board and shareholder approval.

- A cash dividend changes a company's stock price. It needs board approval and distributes earnings to certain shareholders. For example, assume company ABC's board of directors approves a \$2 cash dividend. On the ex-dividend date, company ABC's stock price would reflect the corporate action and would be \$2 less than its previous closing price.
...
- A reverse split would be implemented by a company that wants to force up the price of its shares. For example, if a shareholder owns 10 shares at \$1 each, a 10-for-1 reverse split will give them one \$10 share. A reverse split can be a sign that the company's stock has sunk so low that its executives want to shore up the price, or at least make it appear that the stock is stronger. The company may even need to avoid getting categorized as a penny stock. In other cases, a company may be using a reverse split to drive out small investors
