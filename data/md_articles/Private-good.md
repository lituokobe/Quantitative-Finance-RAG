# Understanding Private Goods: Key Differences From Public Goods
...
We encounter private goods every day. Examples include a dinner at a restaurant, a grocery shopping, airplane rides, and cellphones. A private good is any item that one party can use or consume at a time. Many tangible home goods qualify because only those who have access to them can use them. Items like food and toilet paper, which are used up or destroyed, are also private goods.

Private goods often have limited availability, making them excludable by restricting others' access. For example, only a certain number of a certain pair of designer shoes are produced, so not everyone can have those shoes even if they wish to purchase them. Not only is a single pair seen as a private good, but the entire product line can be classified as such.

The majority of private goods must be purchased for a cost. The cost compensates for the fact that only one person can use the good at a time. Purchasing the item secures the right to consume it and compensates the producer for the costs involved in making it.
...
Private goods are essential components of economic consumption, characterized by their rivalrous and excludable nature. Unlike public goods, which are accessible to everyone, private goods must be purchased and are limited to the consumer's possession and use.
