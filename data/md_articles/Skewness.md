...
Two methods to measure skewness include Pearson’s first and second coefficients of skewness. Pearson’s first coefficient of skewness, or Pearson's mode skewness, subtracts the mode from the mean and divides the difference by the standard deviation.
...
$$\text{Pearson's Skewness} = \frac{Sk_1}{Sk_2} = \frac{\frac{\bar{X}-Mo}{s}}{\frac{3*(X-Md)}{s}}$$
where:
- $Sk_1=\text{Pearson's first coefficient of skewness and Sk_2 the second}$
- $s=\text{The standard deviation for the sample}$
- $\bar{X}=\text{Is the mean value}$
- $Mo=\text{The modal (mode) value}$
- $Md=\text{the median value}$
